...
Item {
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    ( source "$HOME/.config/hypr/scripts/matugen_cmd.sh" && run_matugen || true; bash "$RELOAD_SCRIPT" || true ) &
                `;
        ...
        const fullScript = `
            ...
            ${wallpaperCmd}
            ( source "$HOME/.config/hypr/scripts/matugen_cmd.sh" && run_matugen || true; bash "${escReload}" || true ) &
        `;
        Quickshell.execDetached(["bash", "-c", fullScript]);
    }
    
    function openFilePicker() {
        console.log("[WallpaperPicker] Opening file picker...");
        
        // Use zenity to open file picker
        const pickerScript = `
            SELECTED=$(zenity --file-selection --title='Select Wallpaper' \
                --file-filter='Images | *.jpg *.jpeg *.png *.webp *.gif' \
                --file-filter='Videos | *.mp4 *.mkv *.mov *.webm' \
                --file-filter='All files | *' 2>/dev/null)
            
            if [ -n "$SELECTED" ] && [ -f "$SELECTED" ]; then
                # Copy selected file to wallpaper directory
                DEST_DIR="${window.srcDir}"
                FILENAME=$(basename "$SELECTED")
                cp "$SELECTED" "$DEST_DIR/$FILENAME"
                echo "$FILENAME"
            fi
        `;
        
        Quickshell.execDetached(["bash", "-c", pickerScript]);
    }
    ...
    readonly property string srcDir: "/home/lunanoir/.wallpaper-engine"
    ...
    Rectangle {
        ...
        Row {
            ...
            Rectangle {
                ...
                radius: window.s(10)
                anchors.verticalCenter: parent.verticalCenter
                color: addImageMouse.containsMouse ? _theme.surface2 : "transparent"
                border.color: _theme.surface1
                border.width: 1
                scale: addImageMouse.containsMouse ? 1.08 : 1.0
                ...
                Canvas {
                    ...
                    anchors.centerIn: parent
                    property string iconColor: _theme.text
                ...
                MouseArea {
                    ...
                    enabled: !window.isApplying
                    cursorShape: Qt.PointingHandCursor
                    onClicked: window.openFilePicker()
                }
            }
