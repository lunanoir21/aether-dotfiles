...
Item {
    ...
    // --- INIT CAFFEINE STATE FROM CACHE ---
    Process {
        id: caffeineInit
        running: true
        command: ["bash", "-c", "cat " + paths.getCacheDir("caffeine") + "/state 2>/dev/null || echo '0'"]
        stdout: StdioCollector {
            onStreamFinished: {
                window.caffeineEnabled = (this.text.trim() === "1");
                if (window.caffeineEnabled) {
                    Quickshell.execDetached(["bash", "-c", "hypridle -q & echo $! > " + paths.getCacheDir("caffeine") + "/hypridle_pid"]);
                }
            }
        }
    }
    ...
    ParallelAnimation {
        ...
        // Caffeine mode slides in
        SequentialAnimation {
            PauseAnimation { duration: 500 }
            NumberAnimation { target: window; property: "introCaffeine"; from: 0; to: 1.0; duration: 825; easing.type: Easing.OutBack; easing.overshoot: 0.9 }
        }
    ...
    ParallelAnimation {
        ...
        NumberAnimation { target: window; property: "introCaffeine"; to: 0; duration: 175; easing.type: Easing.InQuart }
            ...
            RowLayout {
                ...
                Quickshell.execDetached(["sh", "-c", "echo 'close' > " + paths.runDir + "/widget_state"]);
                    ...
                    ColumnLayout {
                        ...
                        // 3. CAFFEINE MODE DOCK
                        Rectangle {
                            Layout.fillWidth: true
                            Layout.preferredHeight: window.s(54)
                            radius: window.s(14)
                            color: window.caffeineEnabled ? Qt.alpha(window.teal, 0.15) : (caffeineMa.containsMouse ? window.surface1 : window.surface0)
                            border.color: window.caffeineEnabled ? window.teal : (caffeineMa.containsMouse ? window.surface2 : window.surface1)
                            border.width: 1

                            opacity: introCaffeine
                            transform: Translate { y: window.s(20) * (1.0 - introCaffeine) }

                            Behavior on color { ColorAnimation { duration: 300 } }
                            Behavior on border.color { ColorAnimation { duration: 300 } }

                            scale: caffeineMa.pressed ? 0.98 : (caffeineMa.containsMouse ? 1.02 : 1.0)
                            ...
                            RowLayout {
                                ...
                                Text {
                                    font.family: "Iosevka Nerd Font"
                                    font.pixelSize: window.s(24)
                                    color: window.caffeineEnabled ? window.teal : (caffeineMa.containsMouse ? window.text : window.overlay0)
                                    text: "󰛊"
                                    Behavior on color { ColorAnimation { duration: 200 } }
                                }
                                ...
                                ColumnLayout {
                                    ...
                                    Text {
                                        ...
                                        font.weight: Font.Black
                                        font.pixelSize: window.s(14)
                                        color: window.caffeineEnabled ? window.text : (caffeineMa.containsMouse ? window.text : window.subtext0)
                                        text: "Caffeine Mode"
                                        Behavior on color { ColorAnimation { duration: 200 } }
                                    }
                                    ...
                                    Text {
                                        ...
                                        font.weight: Font.Medium
                                        font.pixelSize: window.s(11)
                                        color: window.caffeineEnabled ? window.teal : window.overlay0
                                        text: window.caffeineEnabled ? "Screen stays awake" : "Keep screen on"
                                        Behavior on color { ColorAnimation { duration: 200 } }
                                    }
                                }
                                ...
                                Rectangle {
                                    ...
                                    Layout.preferredHeight: window.s(26)
                                    radius: window.s(13)
                                    color: window.caffeineEnabled ? window.teal : window.surface2
                                    Behavior on color { ColorAnimation { duration: 300 } }
                                    ...
                                    Rectangle {
                                        ...
                                        color: window.crust
                                        anchors.verticalCenter: parent.verticalCenter
                                        x: window.caffeineEnabled ? parent.width - width - window.s(3) : window.s(3)
                            ...
                            MouseArea {
                                ...
                                cursorShape: Qt.PointingHandCursor
                                onClicked: {
                                    window.caffeineEnabled = !window.caffeineEnabled;
                                    
                                    // Save state to cache
                                    Quickshell.execDetached(["sh", "-c", "echo '" + (window.caffeineEnabled ? "1" : "0") + "' > " + paths.getCacheDir("caffeine") + "/state"]);
                                    
                                    if (window.caffeineEnabled) {
                                        // Kill hypridle to prevent screen timeout
                                        Quickshell.execDetached(["bash", "-c", "pkill -9 hypridle"]);
                                    } else {
                                        // Restart hypridle
